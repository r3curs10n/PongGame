...
namespace App2
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        {
            _gameLoop.Duration = new Duration(new TimeSpan(0));
        ...
        void GameLogic()
        {
            ...
            if (Canvas.GetLeft(ball) > _mx || Canvas.GetLeft(ball) < 0)
                { vx *= -1; s2++; score2.Text = s2.ToString(); }

            if (Canvas.GetTop(ball) > _my || Canvas.GetTop(ball) < 0)
                vy *= -1;
